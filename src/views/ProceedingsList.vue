    ...
    <div class="content-wrapper">
      ...
      <div class="filter-section">
        <div class="search-box">
          <input type="text" v-model="searchQuery" placeholder="搜索标题、作者或关键词..." class="search-input">
          <button class="search-btn" @click="handleSearch">搜索</button>
        </div>
        <div class="filter-tabs">
          <div class="tab" :class="{ active: currentType === 0 }" @click="handleTypeChange(0)">全部</div>
          <div class="tab" :class="{ active: currentType === 1 }" @click="handleTypeChange(1)">论文</div>
          <div class="tab" :class="{ active: currentType === 2 }" @click="handleTypeChange(2)">报告</div>
          <div class="tab" :class="{ active: currentType === 3 }" @click="handleTypeChange(3)">海报</div>
        </div>
      </div>
      ...
      <div class="papers-list">
        <div v-for="paper in paperList" :key="paper.id" class="paper-item">
          ...
          <div class="paper-content">
            ...
            <p class="keywords"><span>关键词：</span>{{ paper.keywords }}</p>
...
import { getThesisListApi, getGraduatesListApi, getConferenceByYearApi } from '../api'
...
const fetchConferenceInfo = async () => {
  try {
    const res = await getConferenceByYearApi(year.value)
    if (res && res.title) {
      conferenceTitle.value = res.title
    } else {
